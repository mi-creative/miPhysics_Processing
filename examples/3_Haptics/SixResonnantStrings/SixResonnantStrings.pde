...
void keyPressed() {
  if (key =='a')
    simUGen.mdl.changeDistParamOfSubset(0.1, "springs");
  else if (key =='a')
    simUGen.mdl.changeDistParamOfSubset(1, "springs");
  else if (key =='z')
    simUGen.mdl.changeDistParamOfSubset(2, "springs");
  else if (key =='e')
    simUGen.mdl.changeDistParamOfSubset(3, "springs");
  else if (key =='r')
    simUGen.mdl.changeDistParamOfSubset(5, "springs");
  else if (key =='t')
    simUGen.mdl.changeDistParamOfSubset(7, "springs");
  else if (key =='y')
    simUGen.mdl.changeDistParamOfSubset(9, "springs");    
  if (keyCode == LEFT) {
  }
}
...
void drawPoint(Vect3D pos, int size) {
  pushMatrix();
  strokeWeight(size);
  translate(100*(float)pos.x, 100*(float)pos.y, 100*(float)pos.z);
  point(0, 0, 0);
  strokeWeight(1);
  popMatrix();
}
