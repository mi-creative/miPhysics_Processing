...
public class PhyUGen extends UGen
{
  ...
  public PhyUGen(int sampleRate)
  {
    ...
    this.mdl.addMass3D("bigmass", m_end, new Vect3D(dist*(nbmass+1),0.0,0.000), new Vect3D(-5,0.1,0.));
    ...
    this.mdl.addSpringDamper3D("sprdg1", l0, k, z, "bigmass", "str"+(nbmass-1));
    
    for(int i= 0; i< nbmass; i++)
      this.mdl.addContact3D("col", c_dist, c_k, c_z, "hapticMass", "str"+i);
    this.mdl.addContact3D("col", c_dist, c_k, c_z, "hapticMass", "bigmass");
