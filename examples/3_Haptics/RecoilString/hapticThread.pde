...
class SimulationThread implements Runnable {
  ...
  public void run() {
    ...
    if(haplyBoard.data_available()){
      ...
      /* set position of the vitual haptic device*/
      synchronized(lock) { 
        simUGen.mdl.setHapticPosition("hapticMass", new Vect3D(posEE.x, posEE.y, 0));
      }

      /* apply virtual force to the real haptic device */
      synchronized(lock) { 
        frcOut = simUGen.mdl.getHapticForce("hapticMass");
      }
