...
void createShapeArray(PhysicalModel mdl) {
  for ( int i = 0; i < mdl.getNumberOfMats(); i++) {
    ...
    case Mass3D:
      addColoredShape(mdl.getMatPosAt(i).toPVector(), color(250, 120, 25), 40);
      break;
    case Mass2DPlane:
      addColoredShape(mdl.getMatPosAt(i).toPVector(), color(200, 170, 200), 5);
      break;
    case Ground3D:
      addColoredShape(mdl.getMatPosAt(i).toPVector(), color(40, 80, 120), 25);
      break; 
    case HapticInput3D:
      addColoredShape(mdl.getMatPosAt(i).toPVector(), color(255, 10, 10), 40);
      break; 
    case Osc3D:
      addColoredShape(mdl.getMatPosAt(i).toPVector(), color(255, 0, 255), 40);
      break;
    case UNDEFINED:
      break;
    }
