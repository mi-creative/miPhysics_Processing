PhyModel createCymbal(int dimX, int dimY, String mName, String lName, double masValue, double dist, double K, double Z) {
  ...
  for (int i = 0; i < dimX-1; i++) {
    for (int j = 0; j < dimY; j++) {
      ...
      mdl.addInteraction(lName + "2_" +i+"_"+j, new SpringDamper3D(dist*0.7, K, Z), masName1, masName2);
    }
  }
  ...
  mdl.addInOut("list_1", new Observer3D(filterType.HIGH_PASS), "osc12_17");
  mdl.addInOut("list_2", new Observer3D(filterType.HIGH_PASS), "osc13_17");
